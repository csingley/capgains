...
class ChangeSecurityReTestCase(CorpActReTestCase):
    ...
    @property
    def descriptions(self):
        return [regexes.corpActRE.match(desc).group('memo').strip()
                for desc in corpActDescriptions if self.memoSignature in desc]
...
class OversubscribeReTestCase(ChangeSecurityReTestCase):
    ...
    def testRegex(self):
        ...
        for i, desc in enumerate(self.descriptions):
            split = list(itertools.chain.from_iterable(
                [m.split(')') for m in desc.split('(')]))
            self.assertEqual(len(split), 3)
            ticker, isin, priceCurrency = split
            ticker = ticker[ticker.index(self.memoSignature)
                            + len(self.memoSignature):]
